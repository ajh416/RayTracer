...
namespace Utils
{
	...
	template<typename T>
	inline Vector3<T> Clamp(const Vector3<T> vec, const Vector3<T> min, const Vector3<T> max)
	{
	...
	inline Vector3<uint8_t> Vec3FloatToVec3Byte(Vector3<Float> vec)
	{
	...
	inline uint32_t VectorToUInt32(Vector3<Float> vec)
	{
