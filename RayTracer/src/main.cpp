...
int main()
{
	...
	renderer.SetSettings({ .NumberOfSamples = 1, .NumberOfBounces = 5, .Accumulate = true, .AccumulateMax = 25 });

	// Create image scene and camera
	Image img(image_width, image_height, 4);
	Camera cam(image_width, aspect_ratio, { 0, 2, 5 });
	Scene scene;
	...
	scene.Shapes.push_back(new Sphere({ -3.0f, 7.0f, -10.0f }, 5.0f, 3));
	...
	scene.Materials.push_back(Material(
		{ 
			...
			.Roughness = 0.5f,
			.Metallic = 0.0f,
			.EmissionColor = Vec3f(1.0f, 0.5f, 0.5f),
			.EmissionStrength = 1.0f
		}));
	scene.Materials.push_back(Material({ {0.6f, 0.6f, 0.8f}, 0.1f, 0.0f, Vec3f(0.0f), 0.0f }));
	scene.Materials.push_back(Material({ {0.8f, 0.4f, 0.4f}, 0.05f, 0.0f, Vec3f(0.0f), 0.0f }));
	scene.Materials.push_back(Material({ {0.8f, 0.4f, 0.4f}, 0.5f, 0.0f, Vec3f(1.0f), 0.9f }));
